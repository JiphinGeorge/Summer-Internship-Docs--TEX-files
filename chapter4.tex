...
Figure \ref{fig:smart_cam} illustrates the final web application interface, demonstrating how users can upload an image to instantly receive classification results alongside detailed confidence metrics.
...
Image Acquisition $\rightarrow$ Image Preprocessing $\rightarrow$ Deep Learning Model $\rightarrow$ Prediction.
...
The backend architecture was developed using the \cite{flask} Flask framework, organized using Blueprints for modularity. REST APIs were developed to handle core services, including Prediction, Dashboard, History, and Report APIs.
...
The AI-Based Live Tomato Freshness Detection System was developed using a combination of Python-based technologies to automate the freshness inspection of tomatoes. The implementation consists of image preprocessing, AI-based prediction, dashboard visualization, alert generation, and inspection history management.

\textbf{Step 1: Dataset Preparation}

A custom dataset containing images of fresh and rotten tomatoes was collected under different lighting conditions, backgrounds, and viewing angles from public sources like Kaggle. The dataset was divided into training, validation, and testing sets to develop a reliable prediction model.
...
The prepared dataset was used to train an AI model capable of identifying the visual differences between fresh and rotten tomatoes. After training, the model was verified using test images before being integrated into the application. Explainable AI (Grad-CAM) was also implemented to highlight important image regions.

\textit{Technology Used: TensorFlow, Keras}

\vspace{0.3cm}

\textbf{Step 4: Backend Implementation}

Python was used to develop the application logic using the Flask framework. During execution, the application automatically processes the system input and passes it to the trained AI model. Based on the prediction, the application classifies the object as Fresh, Rotten, or Unknown Object.

\textit{Technology Used: Python, Flask, TensorFlow}

\vspace{0.3cm}

\textbf{Step 5: Dashboard Development}

An interactive dashboard was developed using HTML5, CSS3, Tailwind CSS, JavaScript, and Chart.js to display the prediction results. After every prediction, the dashboard automatically updates by showing the classification result, confidence score, inspection details, and production statistics.

\textit{Technology Used: HTML5, Tailwind CSS, JavaScript, Chart.js}

\vspace{0.3cm}

\textbf{Step 6: Rotten Alert Notification}

When the application detects a rotten tomato, it automatically triggers an alert sound and displays the corresponding warning on the dashboard. This feature provides immediate notification to the user.

\textit{Technology Used: pyttsx3, Flask-SocketIO}

\vspace{0.3cm}

\textbf{Step 7: Inspection History}

Every prediction made by the application is automatically stored in the Inspection History database using SQLite. The stored information includes the inspection ID, timestamp, prediction result, confidence score, and inspection time. This allows users to review previous inspection results whenever required.

\textit{Technology Used: Python, SQLite}
...
\begin{itemize}[leftmargin=1.5cm]
    \item ACCEPTED -- FRESH for fresh tomatoes.
    \item REJECTED -- ROTTEN for rotten tomatoes.
    \item UNKNOWN OBJECT if the detected object is not recognized as a tomato.
\end{itemize}

The dashboard also displays the confidence score, inspection details, and automatically updates the Inspection History. If a rotten tomato is detected, the system additionally triggers the alert sound.
...
\caption{QualiVision AI Project Pipeline}
\label{fig:pipeline}
\end{figure}

\subsection{Testing and Results}

The developed application was tested using multiple scenarios including Image Upload, Webcam Inspection, Unknown Object Detection, and Dashboard Updates. Functional testing confirmed that all modules worked correctly and stable database operations were maintained.

\textbf{Explainable AI (Grad-CAM):} The system successfully highlights the specific regions of the tomato that strongly influenced the neural network's prediction. This transparency allows operators to understand exactly why a product was flagged as defective.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{images/04-inspection.png}
\caption{Grad-CAM Visualization on Defective Produce}
\label{fig:gradcam}
\end{figure}

\textbf{Dashboard Monitoring:} The Executive Dashboard successfully visualizes the live status of the production line. It dynamically plots analytics, production statistics, and logs the real-time prediction pipeline, ensuring automated monitoring without manual intervention.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{images/02-dashboard.png}
\caption{QualiVision AI - Executive Dashboard}
\label{fig:dashboard}
\end{figure}

\textbf{Fresh/Rotten Detection \& Live Monitoring:} The system accurately detected fresh and rotten tomatoes, instantly updating the live monitoring feed. It successfully triggers the alert and logs the confidence percentage into the database, proving highly reliable for automated inspection.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{images/03-live-monitoring.png}
\caption{QualiVision AI - Live Monitoring}
\label{fig:live_monitoring}
\end{figure}

The AI model was also tested using completely unseen tomato images collected from external sources to verify its generalization capability. Performance testing confirmed fast inference times, proving that fast inference combined with an intuitive industrial dashboard provides substantial automation benefits.
